    ...
    var _mediaStream,
        ...
        _callbacks = {
            ...
            onStreamAdded: function (connection, event) {
                ...
                STes.push(event.stream);
                var id = STes.indexOf(event.stream);
              
                var html = "<div class='span6'> <h4>مخاطب</h4> <video id='" + id +"' class='video partner cool-background' autoplay='autoplay'></video>  </div>"
                
                var VHolder = document.getElementById('videoHolder');
                VHolder.innerHTML += html;
                
                var ListOfVideo = document.querySelectorAll(".video");
                for (var i = 0; i < ListOfVideo.length; ++i) {
                    
                    attachMediaStream(ListOfVideo[i], STes[i]);
                    console.log(i + "");
